...
export function fitSigmoid(hues, responses, polarity, tailProbability = 0.2) {
  ...
  console.log(hues, responses, polarity, tailProbability);
  ...
  for (let iter = 0; iter < 20; iter++) {
    ...
    const grad_a =
      hues.reduce((sum, hue, i) => {
        const z = a * (hue - HUES.mid + b)
        const s = sigmoid(z)
        return sum + (responses[i] - s) * (hue - HUES.mid + b)
      }, 0) -
    ...
    const hess_aa =
      hues.reduce((sum, hue) => {
        const z = a * (hue - HUES.mid + b)
        const s = sigmoid(z)
        return sum - s * (1 - s) * (hue - HUES.mid + b) ** 2
      }, 0) -
      1 / priorA.sd ** 2
    const hess_ab = hues.reduce((sum, hue) => {
      const z = a * (hue - HUES.mid + b)
      const s = sigmoid(z)
      return sum - s * (1 - s) * a * (hue - HUES.mid + b)
    }, 0)
    const hess_bb =
      hues.reduce((sum, hue) => {
        const z = a * (hue - HUES.mid + b)
  ...
  let newProbe = HUES.mid - b + Math.log(percentile / (1 - percentile)) / a
  newProbe = Math.max(HUES.low, Math.min(newProbe + Math.random() * 2 - 1, HUES.high))
